...
namespace Plus.EventBus
{
    ...
    public class GenericEventNameAttribute : Attribute, IEventNameProvider
    {
        ...
        public virtual string GetName(Type eventType)
        {
            ...
            {
                throw new PlusException($"Given type is not generic: {eventType.AssemblyQualifiedName}");
            }

            var genericArguments = eventType.GetGenericArguments();
            if (genericArguments.Length > 1)
            {
                throw new PlusException($"Given type has more than one generic argument: {eventType.AssemblyQualifiedName}");
            }
