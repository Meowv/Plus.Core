...
namespace Plus.IdentityModel
{
    ...
    public class IdentityModelAuthenticationService : IIdentityModelAuthenticationService, ITransientDependency
    {
        ...
        protected PlusIdentityClientOptions ClientOptions { get; }
        ...
        public IdentityModelAuthenticationService(
            IOptions<PlusIdentityClientOptions> options,
        ...
        public virtual async Task<string> GetAccessTokenAsync(IdentityClientConfiguration configuration)
        {
            ...
            {
                throw new PlusException($"Could not retrieve the OpenId Connect discovery document! ErrorType: {discoveryResponse.ErrorType}. Error: {discoveryResponse.Error}");
            }
            ...
            if (tokenResponse.IsError)
            {
                ...
                {
                    throw new PlusException($"Could not get token from the OpenId Connect server! ErrorType: {tokenResponse.ErrorType}. Error: {tokenResponse.Error}. ErrorDescription: {tokenResponse.ErrorDescription}. HttpStatusCode: {tokenResponse.HttpStatusCode}");
                }
                ...
                throw new PlusException(withoutInnerException[0]);
            }
            ...
            IdentityClientConfiguration configuration)
        {
            using (var httpClient = HttpClientFactory.CreateClient(HttpClientName))
            {
                ...
                switch (configuration.GrantType)
                {
                    ...
                    default:
                        throw new PlusException("Grant type was not implemented: " + configuration.GrantType);
                }
            }
        }
