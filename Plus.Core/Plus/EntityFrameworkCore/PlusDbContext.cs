...
using Plus.EntityFrameworkCore.EntityHistory;
using Plus.EntityFrameworkCore.Modeling;
using Plus.EntityFrameworkCore.ValueConverters;
using Plus.Guids;
...
{
    public abstract class PlusDbContext<TDbContext> : DbContext, IPlusEfCoreDbContext, ITransientDependency
        where TDbContext : DbContext
    {
        ...
        public ILogger<PlusDbContext<TDbContext>> Logger { get; set; }

        private static readonly MethodInfo ConfigureBasePropertiesMethodInfo
            = typeof(PlusDbContext<TDbContext>)
                .GetMethod(
        ...
        protected PlusDbContext(DbContextOptions<TDbContext> options)
            : base(options)
        {
            GuidGenerator = SimpleGuidGenerator.Instance;
            EntityChangeEventHelper = NullEntityChangeEventHelper.Instance;
            EntityHistoryHelper = NullEntityHistoryHelper.Instance;
            Logger = NullLogger<PlusDbContext<TDbContext>>.Instance;
        }
        ...
        public virtual void Initialize(PlusEfCoreDbContextInitializationContext initializationContext)
        {
        ...
        protected virtual EntityChangeReport ApplyPlusConcepts()
        {
            ...
            {
                ApplyPlusConcepts(entry, changeReport);
            }

            return changeReport;
        }

        protected virtual void ApplyPlusConcepts(EntityEntry entry, EntityChangeReport changeReport)
        {
            switch (entry.State)
            {
                case EntityState.Added:
                    ApplyPlusConceptsForAddedEntity(entry, changeReport);
                    break;
                case EntityState.Modified:
                    ApplyPlusConceptsForModifiedEntity(entry, changeReport);
                    break;
                case EntityState.Deleted:
                    ApplyPlusConceptsForDeletedEntity(entry, changeReport);
                    break;
            }
        ...
        protected virtual void ApplyPlusConceptsForAddedEntity(EntityEntry entry, EntityChangeReport changeReport)
        {
            CheckAndSetId(entry);
        ...
        protected virtual void ApplyPlusConceptsForModifiedEntity(EntityEntry entry, EntityChangeReport changeReport)
        {
        ...
        protected virtual void ApplyPlusConceptsForDeletedEntity(EntityEntry entry, EntityChangeReport changeReport)
        {
            ...
            where TEntity : class
        {
                ...
                !mutableEntityType.IsOwned())
            {
                ...
                var dateTimeValueConverter = new PlusDateTimeValueConverter(Clock);
