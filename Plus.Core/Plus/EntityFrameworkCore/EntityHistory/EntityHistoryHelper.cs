    ...
    public class EntityHistoryHelper : IEntityHistoryHelper, ITransientDependency
    {
        ...
        private DateTime GetChangeTime(EntityChangeInfo entityChange)
        {
            ...
            switch (entityChange.ChangeType)
            {
                ...
                default:
                    throw new PlusException($"Unknown {nameof(EntityChangeInfo)}: {entityChange}");
            }
        }
        ...
        private string GetEntityId(object entityAsObj)
        {
            ...
            {
                throw new PlusException($"Entities should implement the {typeof(IEntity).AssemblyQualifiedName} interface! Given entity does not implement it: {entityAsObj.GetType().AssemblyQualifiedName}");
            }
